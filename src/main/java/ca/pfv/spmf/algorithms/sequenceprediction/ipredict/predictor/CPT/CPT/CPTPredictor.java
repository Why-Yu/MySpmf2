...
public class CPTPredictor extends Predictor {
	...
	private Map<Integer, Float> CountTable; // Count table
	 ...
	 */
	private void UpdateCountTable(Item[] targetArray, float weight,
								  Map<Integer, Float> CountTable, HashSet<Integer> hashSidVisited,
								  int distance) {
		...
		for(int index = indexes.nextSetBit(0); index >= 0 ; index = indexes.nextSetBit(index+1)) {
			...
			for(i = 0; i <= consequentEndPosition; i++) {
				...
				float curValue = (1f /((float)indexes.cardinality())) * (countList.get(i) );
				CountTable.put(branch.get(i).val, oldValue + (curValue * weight) / distance );

//				float curValue = (1f /((float)indexes.cardinality())) * (weight * 10 + (1.0f / distance) * 10 + countList.get(i) * 0.1f);
//				CountTable.put(branch.get(i).val, oldValue + curValue );
	...
	public Sequence Predict(Sequence target) {
		...
		if(initialTargetArraySize == 1){
			...
			UpdateCountTable(targetArray, weight, CountTable, hashSidVisited, 1);
			// ==== END OF NEW CASE
		}else{
			...
			for(i = minRecursion ; i < maxRecursion && prediction.size() == 0; i++) {
				...
				bitSet.set(0,targetArray.length,true);

				//Dividing the target sequence into sub sequence
				RecursiveDivider(targetArray, minSize, CountTable, hashSidVisited, initialTargetArraySize, bitSet);
			}
		}
	...
	/**
	 * Obtain the count table used for the latest prediction 
	 * made by the model.
	 * @return a map where the key is a symbol and the value is a score assigned
	 *    by the model.  The item with the highest score is the prediction.
	 *    Otherwise, null, if no prediction was made.
	 */
	 ...
	 */
	public void RecursiveDivider(Item[] targetArray, int minSize, Map<Integer, Float> countTable,
								 HashSet<Integer> hashSidVisited , int initialTargetArraySize,
								 BitSet bitSet) {
		...
		int distance = initialTargetArraySize - lastBit;
		
		//Setting up the weight multiplier for the countTable
		float weight = (float)size / initialTargetArraySize;
		
		UpdateCountTable(targetArray, weight, countTable, hashSidVisited, distance);
...
//			}
			...
			if (toHide == 0) {
				newBitSet.set(newBitSet.nextSetBit(0), false);
			} else {
				int count = 0;
				int hideBit = newBitSet.nextSetBit(0);
				while (count < toHide) {
					hideBit = newBitSet.nextSetBit(hideBit + 1);
					count++;
				}
				newBitSet.set(hideBit, false);
			}
			...
			RecursiveDivider(newSequence, minSize, countTable, hashSidVisited, initialTargetArraySize, bitSet);
		}
	}
...
//					curNode.addChild(it);
					curNode.addChild(it, curSeq.count);
					nodeNumber++;
				}
