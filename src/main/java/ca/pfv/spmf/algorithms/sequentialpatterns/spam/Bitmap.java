...
//					bitK = lastBitOfSID; // to skip the bit from the same sequence
	...
	Bitmap noobCreateNewBitmapSStep(Bitmap bitmapItem, List<Integer> sequencesSize, int lastBitIndex, int maxGap) {
		// create a new bitset that will be use for the new bitmap
//		Bitmap newBitmap = new Bitmap(new BitSet(lastBitIndex));
		Bitmap tempBitmap = new Bitmap(new BitSet(lastBitIndex));
		...
		if(maxGap == Integer.MAX_VALUE) {
			...
			for (int bitK = bitmap.nextSetBit(0); bitK >= 0; bitK = bitmap.nextSetBit(bitK+1)) {
				...
				// get the index of the last bit representing this sequence (sid)
				int lastBitOfSID = lastBitOfSID(sid, sequencesSize, lastBitIndex);
				tempBitmap.bitmap.set(bitK + 1, lastBitOfSID + 1);
				bitK = lastBitOfSID;
			}

			tempBitmap.bitmap.and(bitmapItem.bitmap);

			if (tempBitmap.bitmap.cardinality() > 0) {
				for (int bit = tempBitmap.bitmap.nextSetBit(0); bit >= 0 ; bit = tempBitmap.bitmap.nextSetBit(bit + 1)) {
					// update the support
					int sid = bitToSID(bit, sequencesSize);
					if(sid != tempBitmap.lastSID){
						tempBitmap.support++;
						tempBitmap.supportWithoutGapTotal++;
						tempBitmap.sidsum += sid;
						tempBitmap.lastSID = sid;
					}
				}
			}
		}
